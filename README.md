<h1 align = center>StrongPlanet 🌱💪🏻💪🏿🌱</h1>

<h2 align = center>Desenvolvimento de esteiras sustentáveis para academias!</h2>

<h3 align = center>Nos dias de hoje, estamos precisando cada vez mais que as pessoas se familizarizem com a prática de exercícios e, principalmente, de se conscientizarem sobre os cuidados do meio ambiente. Portanto, unimos o melhor dos dois mundos e o objetivo deste projeto é criar uma esteira que transforma a energia mecânica do usuário em energia elétrica. Criando um novo estilo de vida que contribui para a saúde das pessoas e do planeta</h3>

## Escopo do Projeto

### Recursos disponíveis:
+ Equipe pequena
+ Orçamento razoável
+ Uma pessoa responsável pelo SM, PO e a gerência do projeto

### Funcionalidades essenciais:
+ Esteira que se mexe no intuito de fazer o usuário andar ou correr

+ Um transformador de energia que transformará a energia mecânica em elétrica

+ O equipamento terá uma bateria interna que se auto alimenta com o esforço da pessoa, parte da energia gerada vai para essa bateria.

+ Um painel que conta a frequência cardíaca, calorias consumidas, quantos quilômetros corridos, distância percorrida, ajustes de inclinação e um  software que faz uma avaliação no mês de quanto foi economizado de energia e quantos Watt/h foram gerados.

### Materiais:
+ Painel com tecnológico
+ Aço inoxidável
+ Bateria que fará a esteira funcionar 
+ Transformador que irá converter a corrida em eletricidade

### Mão de obra:
+ Designers
+ Engenheiros elétricos
+ Arquitetos
+ Desenvolvedores de Software
+ Marketeiros

## Personas 👥
### Marcos 
<img src = "https://github.com/YanYamim/StrongPlanet-FGTI/assets/119015786/94491354-778d-466d-a29a-e76ed7e5708f" width = 300px>

### Carlos
<img src = "https://github.com/YanYamim/StrongPlanet-FGTI/assets/119015786/3e2cd204-0157-4b17-afb4-7945cec43e24" width = 300px>

### Michelle
<img src = "https://github.com/YanYamim/StrongPlanet-FGTI/assets/119015786/68eb3fb2-aadf-4e31-8278-f1f1d41c4db6" width = 300px>


## Orçamento 💰
<img src = "https://github.com/YanYamim/StrongPlanet-FGTI/assets/119015786/5a3be3ad-24a4-4dc4-928a-a6232de03be0" width = 500 px>

## Project Model Canvas 📋
<img align = center src = "https://github.com/YanYamim/StrongPlanet-FGTI/assets/119015786/1074a959-e4d7-449d-94c9-3018a4a03926" width = 700px>

## Product Backlog
| Prioridade | Função |
| --- | --- |
| `Alta` | Desenvolvimento do Sistema de Geração de Energia |
| `Alta` | Motor de Alto Desempenho |
| `Alta` | Monitoramento de Geração de Energia |
| `Alta` | Armazenamento de Energia |
| `Alta` | Sistema de Resfriamento Eficiente |
| `Média` | Design ergonômico |
| `Média` | Eficiência do Motor |
| `Média` | Monitoramento de Consumo de Energia |
| `Média` | Sensores de Bem-Estar |
| `Baixa` | Reciclagem de Componentes |
| `Baixa` | Integração com Fontes de Energia Renovável |
| `Baixa` | Integração com a Rede Elétrica |

## Divisão das Sprints
| Sprint |         Datas        |               Tarefas               |
|:------:|:----------------------:|:----------------------------------:|
|   1    | 1/08 a 15/08           | - Definição da equipe              |
|        |                       | - Eficiência do Motor              |
|        |                       | - Criação de personas              |
|        |                       | - Apresentação do projeto aos investidores     |
|        |                       | - Apresentação do escopo e da equipe aos investidores (6 min)     |
|   2    | 15/08 a 10/09          | - Arquitetura da máquina           |
|        |                       | - Desenvolvimento do Sistema de Geração de Energia     |
|        |                       | - Contato com fornecedores dos materiais     |
|        |                       | - Contato com investidores         |
|   3    | 10/09 a 10/10          | - Motor de Alto Desempenho         |
|        |                       | - Monitoramento de Geração de Energia      |
|        |                       | - Monitoramento de Consumo de Energia     |
|        |                       | - Design ergonômico                |
|        |                       | - Demonstração do motor, design e do uso da energia (6 min)    |
|   4    | 10/10 a 10/11          | - Armazenamento de Energia         |
|        |                       | - Sistema de Resfriamento Eficiente         |
|        |                       | - Sensores de Bem-Estar            |
|        |                       | - Integração com a Rede Elétrica   |
|        |                       | - Apresentação do sistema de resfriamento, sensores e integração elétrica (5 min)      |
|   5    | 10/11 a 25/11         | - Reciclagem de Componentes         |
|        |                       | - Integração com Fontes de Energia |
|        |                       | - Teste das Máquinas               |
|        |                       | - Teste definitivo das máquinas,  apresentação dos componentes recicláveis e seus destinos   |
|   6    | 25/11 a 1/12           | - Contato com distribuidoras       |
|        |                       | - Planejamento da logística de entrega     |
|        |                       | - Campanha de marketing para o lançamento    |
|        |                       | - Apresentação das máquinas às empresas de distribuição (5 min)   |
|   7    | 01/12 a 15/12          | - Lançamento do projeto            |
|        |                       | - Manutenção das máquinas          |
